...
def pick(paragraphs, select, k):
    ...
    new_list = []
    num = 0
    while num < len(paragraphs):
        if select(paragraphs[num]):
            new_list.append(paragraphs[num])
            num +=1
        else:
            num +=1
    if k >= len(new_list):
...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def helper(words):
        list = split(remove_punctuation(lower(words)))
        for i in list:
            for j in topic:
                if i == j:
                    return True
...
def accuracy(typed, reference):
    ...
    if typed_words == [] and reference_words == []:
        ...
        num = len(typed_words)
        num_2 = len(reference_words)
        for i in range(num):
            if i >= num_2:
                total = total + len(typed_words[i:])
