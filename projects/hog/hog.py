...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        score = oink_points(player_score, opponent_score)
    else:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0 and score1 < goal:
            score0 = take_turn(strategy0(score0, score1), score0, score1, dice, goal) + score0
            score0 = score0 + pigs_on_prime(score0, score1)
            who = next_player(who)
        elif who ==1 and score0 < goal:
            score1 = take_turn(strategy1(score1, score0), score1, score0, dice, goal) + score1
            score1 = score1 + pigs_on_prime(score1, score0)
            who = next_player(who)
        c, d = say(score0, score1, leader)
        leader = c
        if d != None:
            print(d)
...
def make_averaged(original_function, total_samples=1000):
    ...
    def new_make_avg(*args):
        result = 0
        count = 1
        while count <= total_samples:
            result = original_function(*args) + result
            count = count +1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    ##number_dice_2 = 2
    count = 2
    total_score = make_averaged(roll_dice, total_samples)(number_dice, dice)
    ##total_score_2 = make_averaged(roll_dice, total_samples)(number_dice_2, dice)
    while count <= 10:
        current_value = make_averaged(roll_dice, total_samples)(count, dice)
        if  current_value > total_score:
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if pigs_on_prime((score + oink_points(score, opponent_score)), opponent_score):
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    score_need = GOAL_SCORE - score
    if oink_points(score, opponent_score) <= score_need:
        return oink_points_strategy(score, opponent_score, score_need, 6)
    elif score_need <= 6:
        return 1
    elif pigs_on_prime(score, opponent_score) > 7:
        return 1
    else:
        return 3
    # Remove this line once implemented.
