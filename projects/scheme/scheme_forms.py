...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(function_name, do_lambda_form(Pair(formals, body), env))
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        if expressions.rest != nil and is_scheme_false(scheme_eval(expressions.first, env)):
...
def do_or_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    while expressions != nil:
        if expressions.rest != nil and is_scheme_true(scheme_eval(expressions.first, env)):
            return scheme_eval(expressions.first, env)
        elif expressions.rest == nil:
            return scheme_eval(expressions.first, env, True)
        else:
